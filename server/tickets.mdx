---
title: "Create a Support Ticket System with XRYPTON"
sidebarTitle: "Tickets"
description: "Build a fully featured support ticket system with topic selection, staff roles, transcript logging, and a persistent panel embed — all powered by XRYPTON."
---
...
<Steps>
  <Step title="Set the ticket category">
    ...
    **Permission:** Administrator  
    New ticket channels will be created inside this category. Omit the argument to remove the category setting.
  </Step>
  ...
  <Step title="Add staff roles">
    ...
    ```
    **Permission:** Administrator  
    Run the command again with the same role to remove it. Run `,ticket staff` with no argument to clear all staff roles. Staff roles are pinged (and immediately deleted) whenever a new ticket is opened.
  </Step>
  <Step title="Set the transcript log channel">
    ...
    ```
    **Permission:** Administrator  
    When a ticket is closed, a `.txt` transcript of the human-readable messages is uploaded to this channel along with a summary embed. Omit the argument to disable logging.
  </Step>
...
2. The ticket is removed from the database.
3. The channel is deleted after a 2-second delay.
