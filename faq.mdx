---
title: "Frequently Asked Questions About XRYPTON Bot"
sidebarTitle: "FAQ"
...
<AccordionGroup>
  ...
  <Accordion title="Why is the bot not responding to my commands?">
    ...
    - **Privileged intents not enabled** — XRYPTON requires all privileged intents to be enabled on the bot application. If you are self-hosting, confirm this in the Discord Developer Portal.
  ...
  <Accordion title="Can I customize the bot's name and avatar per server?">
    Discord does not allow bots to have different names or avatars on a per-server basis — any nickname or avatar change applies account-wide. However, a server administrator **can** give XRYPTON a server-specific nickname via Discord's built-in **Edit Nickname** feature on the bot's profile, which changes how the bot's name appears only within your server.

    Note that changing the bot's nickname does not affect how commands or mentions work.
  </Accordion>
