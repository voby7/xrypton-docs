...
XRYPTON brings AI capabilities straight into your Discord server or DMs. Whether you need a quick answer to a question, a custom-generated image, or an audio clip from written text, all three features are available as simple slash commands. Every AI command works in servers, DMs, and private channels alike.

<CardGroup cols={3}>
  <Card title="/ask" icon="comment-dots">
    Ask the AI any question and receive a text response powered by the `kai/step-3.7` model.
  </Card>
...
Use `/ask` to send a prompt to XRYPTON's AI and receive a conversational response. The bot uses the `kai/step-3.7` model and is set up to be helpful, direct, and unrestricted in its answers.
